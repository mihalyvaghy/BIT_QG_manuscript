...
MFPolynomialQuantumGraph::MFPolynomialQuantumGraph(int N, int vertices, const std::vector<QGEdge>& edges) {
	size = vertices;
	...
	D.resize(sizeG, sizeG);
	D.reserve(sizeG);
	Dinv.resize(sizeG, sizeG);
	Dinv.reserve(sizeG);
	double dii;
	Eigen::VectorXd rhs;
	for (int i = 0; i < sizeG; ++i) {
		rhs = Eigen::VectorXd::Zero(sizeG);
		rhs(i) = 1.0;
		dii = (AGG*rhs-AIG.transpose()*mysolver.solve(AIG*rhs))(i);
		D.insert(i, i) = dii;
		if (dii != 0)
			Dinv.insert(i, i) = 1/dii;
		else
			Dinv.insert(i, i) = 1;
	}

	bG = 2*Dinv*bG-Dinv*(AGG*Dinv*bG-AIG.transpose()*mysolver.solve(AIG*Dinv*bG));
}
